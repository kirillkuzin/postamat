...
func TestComposeDefinesServerPostgresProxyAndTurn(t *testing.T) {
	...
	for _, want := range []string{
		...
		"traefik.http.routers.postamat-https.middlewares=postamat-security-headers",
		"traefik.http.routers.postamat-https.tls.certresolver=letsencrypt",
		"traefik.http.middlewares.postamat-security-headers.headers.stsSeconds=31536000",
		"traefik.http.middlewares.postamat-security-headers.headers.contentTypeNosniff=true",
		"traefik.http.middlewares.postamat-security-headers.headers.referrerPolicy=no-referrer",
		"traefik.http.middlewares.postamat-metrics-no-store.headers.customResponseHeaders.Cache-Control=no-store",
		"traefik.http.routers.postamat-metrics.rule=Host(`${POSTAMAT_PUBLIC_HOST:?set POSTAMAT_PUBLIC_HOST}`) && Path(`/metrics`)",
		"traefik.http.routers.postamat-metrics.middlewares=postamat-security-headers,postamat-metrics-no-store",
		"traefik.http.routers.postamat-metrics.priority=100",
